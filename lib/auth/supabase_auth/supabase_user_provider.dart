...
class LaborTrackSupabaseUser extends BaseAuthUser {
  LaborTrackSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> laborTrackSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = LaborTrackSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
